 ...
 **/
function _helper_active_link($output, $a, $b = 'index') {
//	$CI = &get_instance();

	$output['activeLink'] = array($a => array($b => 1));
	return $output;
	/*
		$CI->template->loadData("activeLink",
			array($a => array($b => 1)));
	*/
}
function _helper_select($a, $value = '') {
	foreach ($a as $k => $v) {
		?>
 	 	  <option <?php if ($v['id'] == $value): ?>
 	 	     	 selected
 	 	  <?php endif?> value="<?php echo $v['id'] ?>"><?php echo $v['value']; ?></option>
 	 	 <?PHP
}
...
function fileUpload_foto($index, $id) {
	$CI = &get_instance();
	//	$CI->load->library('email');
	//	foreach($_FILES as $name => $fileInfo)
	//	{
	$filename = $_FILES['foto_' . $index]['name'];
	$tmpname = $_FILES['foto_' . $index]['tmp_name'];
	$exp = explode('.', $filename);
	$ext = end($exp);
	$newname = $id . "." . $ext;
	$config = array();
	$config['upload_path'] = 'documentos/fotos/';
	$config['upload_url'] = base_url() . 'documentos/fotos/';
...
//	echo "-->id:" . $id . ':::' . $index;
	...
	if (!$CI->upload->do_upload('foto_' . $index)) {
		$data = array('error'); // array('error' => $CI->upload->display_errors());

	} else {

		$data = array("ok"); //array('upload_data' => $CI->upload->data());

	}
//	print_r($config);
	//move_uploaded_file($tmpname, "documentos/fotos/" . $newname);
	return $data;
	//	}
}

function fileUpload_partida($index, $id) {
	$CI = &get_instance();
	//	$CI->load->library('email');
	//	foreach($_FILES as $name => $fileInfo)
	//	{
	$filename = $_FILES['partida_' . $index]['name'];
	$tmpname = $_FILES['partida_' . $index]['tmp_name'];
	$exp = explode('.', $filename);
	$ext = end($exp);
	$newname = $id . "." . $ext;
	$config = array();
	$config['upload_path'] = 'documentos/partidas/';
	$config['upload_url'] = base_url() . 'documentos/partidas/';
	...
	if (!$CI->upload->do_upload('partida_' . $index)) {
		$data = array('error'); // array('error' => $CI->upload->display_errors());
	...
	//print_r($config);
	//move_uploaded_file($tmpname, "documentos/partidas/" . $newname);
	return $data;
	//	}
}
